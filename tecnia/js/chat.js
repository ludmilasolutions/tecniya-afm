...
function renderMsgBubble(msg, isSent) {
  ...
  const isBot   = msg.sender_id === '00000000-0000-0000-0000-0000000000a1';
  ...
  return `<div class="msg ${isSent ? 'sent' : 'received'} ${isBot ? 'bot' : ''}" data-msg-id="${msg.id}">
    ${bubble}
    <div class="msg-meta">${isBot ? '<span class="bot-badge">Bot</span> ' : ''}${time} ${readIcon}</div>
  </div>`;
}
...
// ─── ABRIR CHAT CON USUARIO ESPECÍFICO (Para TecniBot) ─────────────────────────

export async function openChatWith(otherUserId) {
  if (!store.currentUser) {
    showToast('Debes iniciar sesión para hablar con TecniBot', 'info');
    return;
  }
  const sb = getSupabase();
  try {
    const { data: convs, error } = await sb
      .from('conversations')
      .select('id')
      .or(`and(participant_one.eq.${store.currentUser.id},participant_two.eq.${otherUserId}),and(participant_one.eq.${otherUserId},participant_two.eq.${store.currentUser.id})`)
      .maybeSingle();
